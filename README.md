...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    #btn-hr.measuring{background:#ff4d6d22;border:1px solid #ff4d6d;
      animation:pulse 1.4s infinite}
    ...
    #hr-modal{
      ...
      background:rgba(0,0,0,.9);z-index:9000;
    ...
    #modal-close{
      margin-top:20px;width:100%;padding:14px;border-radius:50px;
...
<div id="app">
  ...
  <div id="controls">
    ...
    <button class="btn" id="btn-hr"    onclick="openHR()" title="Medir pulso con cámara">&#x2665;</button>
  </div>
</div>
...
const S = {
  ...
  calories:0,
  bpm:null, bpmSum:0, bpmCount:0,
  positions:[],
  watchId:null, timerIv:null,
...
function calcCal(){
  return Math.round(8*WEIGHT*(elapsedSec()/3600));
}

function onPos(p){
  const {latitude:la,longitude:ln,accuracy:ac,speed:gs}=p.coords;
...
function onGpsErr(){ document.getElementById('gps-acc').textContent='GPS: error'; }
...
function paceZone(kmh){
  ...
  if(kmh<10) return 'ritmo suave';
  if(kmh<13) return 'ritmo medio';
  if(kmh<16) return 'ritmo rápido';
  return 'sprint';
}
const ZONE_MSG={
...
function voiceCheck(){
  const km=S.km, sp=S.speed, sec=elapsedSec();
  const mile=Math.floor(km);
  ...
  if(zone!==S.lastPaceAnn&&sec>12){
    S.lastPaceAnn=zone;
    speak(ZONE_MSG[zone]||zone);
  }
  if(S.bpm){
...
function startRun(){
  ...
  document.getElementById('status-dot').className='active';
  S.timerIv=setInterval(tick,500);
  navigator.geolocation.getCurrentPosition(p=>{onPos(p);startGPS();speak('Carrera iniciada. ¡Buena suerte!');},onGpsErr,{enableHighAccuracy:true,timeout:10000});
}
function pauseRun(){
...
function detectBPM(){
  ...
  const bpm=Math.round(60/(avg/15));
